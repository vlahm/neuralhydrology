...
class BaseDataset(Dataset):
    ...
    def _load_or_create_xarray_dataset(self) -> xarray.Dataset:
        ...
        if (self.cfg.train_data_file is None) or (not self.is_train):
            ...
            for basin in tqdm(self.basins, disable=self._disable_pbar, file=sys.stdout, position=0, leave=True):
    ...
    def _calculate_per_basin_std(self, xr: xarray.Dataset):
        ...
        for basin in tqdm(self.basins, file=sys.stdout, disable=self._disable_pbar, position=0, leave=True):
    ...
    def _create_lookup_table(self, xr: xarray.Dataset):
        ...
        basin_coordinates = xr["basin"].values.tolist()
        for basin in tqdm(basin_coordinates, file=sys.stdout, disable=self._disable_pbar, position=0, leave=True):
